 // 1.输出表格 可以不传参数也可以传 行数 列数 颜色  是否有边框  true  false 如何能只传颜色也可以使用？
     // rows  行数 
     // cols  列数
     // bg    背景颜色
     // border 是否有边框
     function table(rows,cols,bg,border){
      ...
      if(border){document.write("<table border='1'bgcolor="+bg+">");
    }else{document.write("<table bgcolor="+bg+">")}
...
// 2.写一个函数  当传一个参数的时候  返回从1累加的结果  如果传多个参数  返回这些参数加起来的值  如果在这些参数里面有不是数值的  直接返回false
    function fun3(){
    var sum=0;
  ...
  // var val1=fun3(10);
  // var val2=fun3(10,10,20,50);
  // var val3=fun3(10,10,20,50,"abc");
...
function addEvent(obj,type,fn){
	if(obj.addEventListener){
		...
		obj.attachEvent("on"+type,fn.call(obj));
	}
}
